...
todos_anos = st.sidebar.checkbox('Dados de todo i periodo', value=True)
if todos_anos:
    ano = ''
else:
    ano = st.sidebar.slider('Ano', 2020, 2023)

query_string = {'regiao': regiao.lower(), 'ano': ano}  # type: ignore
...
receita_estados = dados.drop_duplicates(
    subset='Local da compra')[['Local da compra',
                               'lat',
...
vendas_mensal = pd.DataFrame(dados.set_index(
    'Data da Compra').groupby(pd.Grouper(
        freq='ME'))['Preço'].count()).reset_index()
...
fig_receita_estados = px.bar(receita_estados.head(),
                             ...
                             text_auto=True,
                             color='Preço',
                             title='Receita por Estado (receita)')
...
tab1, tab2, tab3, tab4 = st.tabs(
    ['Receita', 'Quantidade de Vendas', 'Vendedores', 'Tabela de Dados'])
with tab1:
...
with tab3:
    qtd_vendedores = int(st.number_input('Quantidade de Vendedores', 2, 50, 5))
    coluna1, coluna2 = st.columns(2)
    with coluna1:
        st.metric('Receita Total', formata_numero(dados['Preço'].sum(), 'R$'))
        fig_receita_vendedores = px.bar(vendedores[['sum']].sort_values(
            'sum', ascending=True).head(qtd_vendedores),
            x='sum',
    ...
    with coluna2:
        st.metric('Quantidade de Vendas', formata_numero(dados.shape[0]))
        fig_vendas_vendedores = px.bar(vendedores[['count']].sort_values(
            'count', ascending=True).head(qtd_vendedores),
            x='count',
